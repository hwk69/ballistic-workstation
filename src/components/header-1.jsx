...
/**
 * App header for Ballistic Workstation.
 * Props:
 *   phase        — current app phase (number)
 *   navItems     — array of { label, ph, disabled, onClick }
 *   sessionCount — number of recorded sessions
 *   onExport     — callback for export action
 */
export function Header({ phase, navItems = [], sessionCount = 0, onExport }) {
		...
		<header className={cn(
			...
			<nav className="mx-auto flex h-14 w-full max-w-6xl items-center justify-between px-6">
				...
				<div className="flex items-center gap-2 shrink-0">
					<Crosshair size={16} className="text-primary" strokeWidth={2} />
					<span className="text-primary font-bold text-xs tracking-[0.12em] uppercase">
						Ballistic WS
					</span>
				</div>
				...
				<div className="hidden items-center gap-0.5 md:flex">
					{navItems.map(item => (
						<button
							...
							className={cn(
								'h-8 px-3 rounded-lg text-sm font-medium transition-colors duration-150 cursor-pointer',
			...
			<MobileMenu open={open} className="flex flex-col justify-between gap-2">
				...
				{onExport && (
					<Button variant="outline" className="w-full bg-transparent" onClick={onExport}>
						Export
					</Button>
				)}
			</MobileMenu>
